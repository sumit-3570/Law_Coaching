...
include_once "conn.php";
 ...
 if(isset($submit)) {
        ...
        $sel_admin_sign="SELECT * FROM admin_sign WHERE admin_sign_email ='$registered_email'";
        $exe_admin_sign=mysqli_query($conn,$sel_admin_sign);
        $fetch_admin_sign=mysqli_num_rows($exe_admin_sign);
        if($fetch_admin_sign == 1) {
          $fetch_data_admin=mysqli_fetch_assoc($exe_admin_sign);
          $otp=rand(1000,9999);
          echo $otp;
          $_SESSION['admin_sign_id']=$fetch_data_admin['admin_sign_id'];
